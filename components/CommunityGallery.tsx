...
const CommunityGallery: React.FC<CommunityGalleryProps> = ({ onNavigate, onFork }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
        setIsLoading(true);
        const fetched = await fetchCommunityDiagrams();
  ...
  const filteredData = diagrams.filter(d => 
      ...
      <nav className="h-16 border-b border-border bg-surface/50 backdrop-blur-xl flex items-center justify-between px-6 shrink-0 z-50">
         ...
         <div className="hidden md:flex items-center gap-2 max-w-md w-full mx-4">
            <div className="relative w-full group">
                ...
                <input 
                    type="text" 
                    placeholder="Search diagrams, tags, or authors..." 
                    value={searchQuery}
        ...
        <div className="max-w-7xl mx-auto mb-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                ...
                <div className="flex gap-2 bg-surface border border-border p-1 rounded-xl">
                    <FilterButton active={filter === 'trending'} onClick={() => setFilter('trending')} icon={<TrendingUp className="w-4 h-4" />}>Trending</FilterButton>
                    <FilterButton active={filter === 'new'} onClick={() => setFilter('new')} icon={<Clock className="w-4 h-4" />}>Newest</FilterButton>
                    <FilterButton active={filter === 'top'} onClick={() => setFilter('top')} icon={<Star className="w-4 h-4" />}>Top Rated</FilterButton>
                </div>
            </div>
                        ...
                        >
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-2">
                                    <h3 className="font-bold text-lg text-text group-hover:text-primary transition-colors line-clamp-1">{diagram.title}</h3>
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {diagram.tags.map(tag => (
                                        <span key={tag} className="text-[10px] font-medium text-primary bg-primary/10 px-2 py-1 rounded-md">#{tag}</span>
                                    ))}
                                </div>
            ...
            {!isLoading && (
                <div className="mt-20 p-10 rounded-2xl bg-gradient-to-br from-surface to-surface-hover border border-border text-center">
                    ...
                    <p className="text-text-muted max-w-lg mx-auto mb-6">
                        Publish your best diagrams to the community. Top contributors receive Archigram Pro for free and exclusive profile badges.
                    </p>
                    <button 
                        onClick={() => onNavigate('app')}
